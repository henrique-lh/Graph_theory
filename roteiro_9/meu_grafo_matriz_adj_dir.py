...
class MeuGrafo(GrafoMatrizAdjacenciaDirecionado):
    ...
    def deleta_fonte(self, grafo: MeuGrafo, source_set: Set) -> None:
        """Deleta os vértices fontes de um grafo

        Args:
            grafo (MeuGrafo): Grafo de onde será deletado o vértice
            source_set (Set): Conjunto com vértices fonte
        """
        for vertice in source_set:
            indice_v = grafo.N.index(vertice)
            for linha in grafo.M:
                del linha[indice_v]
            del grafo.N[indice_v]
            del grafo.M[indice_v]

    def busca_vertice_fonte(self, grafo: MeuGrafo, ordem: List[str], source_set: Set) -> None:
        """Busca os vértices fontes em um grafo

        Args:
            grafo (MeuGrafo): Grafo
            ordem (List[str]): Lista com vértices ordenados topologicamente
            source_set (Set): Conjunto que armazena os vértices fontes do grafo
        """
        tamanho_grafo = len(grafo.N)
        for i in range(tamanho_grafo):
            soma = 0
            for j in range(tamanho_grafo):
                soma += len(grafo.M[j][i])
            if soma == 0:
                v = grafo.N[i]
                ordem.append(v)
                source_set.add(v)
    ...
    def khan_algorithm(self) -> List[str]:
        """Armazena os vértices de um grafo em ordem topológica

        Returns:
            List[str]: Lista com vértices em ordem
        """
        ord_top = []
        new_sources = set()
        grafo = deepcopy(self)
        while grafo.N:
            self.busca_vertice_fonte(grafo, ord_top, new_sources)
            self.deleta_fonte(grafo, new_sources)
            new_sources.clear()
        return ord_top
